...
export const useSetupStore = defineStore('setupStore', {
  ...
  actions: {
    ...
    async fetchParametersExceptNormal() {
      try {
        const response = await http.get('/parameters')
        this.parametersWithoutNormal = response.data.filter(p => p.name !== 'normal')
        console.log(this.parametersWithoutNormal)
      } catch (error) {
        console.error('Failed to fetch parameters:', error)
      }
    },
